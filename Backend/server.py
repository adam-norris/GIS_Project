...
@app.route('/api/data/bardichteNorm', methods=["POST"])
def bardichteNorm():
    query = """
        select polygons.osm_id, polygons.name as name, 
            st_asgeojson(polygons.way) as geojson, 
            st_area(polygons.way::geography) / (1000 * 1000) as area, 
            COUNT(points.*) as num_bars, 
            pop as pop, 
            CAST(COUNT(points.*)/pop as float) as norm
        from planet_osm_polygon polygons 
        JOIN planet_osm_point points ON ST_Contains(polygons.way, points.way)
        JOIN pop_data popu ON polygons.name = popu.kreis
        where polygons.admin_level = '6' and polygons.boundary = 'administrative' 
        AND (points.amenity='bar' OR points.amenity='pub')
        group by polygons.osm_id, polygons.name, polygons.way, popu.pop
    """
