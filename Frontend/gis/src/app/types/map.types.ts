...
class BardichteNormLayer extends Overlay {
    ...
    createOverlay() {
        ...
        // .domain expects an array of [min, max] value
        // d3.extent returns exactly this array
        const minMaxBars = d3.extent(this.featureCollection.features.map(d => d.properties.barspop));
        const colorScale = d3.scaleSequential(d3.interpolateReds).domain(minMaxBars);
        ...
        // create geojson layer (looks more complex than it is)
        const bardichteNormLayer = L.geoJSON(this.featureCollection, {
            style: (feature) => {
                return {
                    fillColor: colorScale(feature.properties.barspop),
                    weight: 2,
                    opacity: 1,
                    color: 'white',
                    dashArray: '3',
            ...
            onEachFeature: (feature, layer) => {
                layer.on({
                    ...
                    mouseover: (e: L.LeafletMouseEvent) => {
                        ...
                        tooltip.style('left', `${e.originalEvent.clientX + 25}px`);
                        nameP.text(`County: ${feature.properties.name}`);
                        areaP.text(`Bars/pop: ${feature.properties.barspop}`);
                    ...
                    mouseout: (e: L.LeafletMouseEvent) => {
                        tooltip.style('display', 'none');
                        bardichteNormLayer.resetStyle(e.target);
                    }
                });
            }
        });

        return bardichteNormLayer;
    }
}


export { Overlay, LandkreisLayer, BardichteLayer, BardichteNormLayer };
